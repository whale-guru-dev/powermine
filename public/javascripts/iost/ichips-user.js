...
function updateTimer() {
  ...
  const updateTimer_internal = function () {
    ...
    if (Difference_In_Time > 0) {
      ...
      var Difference_In_Hour = Math.floor(
  ...
  const fetchToken = () => {
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        document.getElementById("token-value").innerText = parseFloat(
          1000 - xhttp.responseText
        ).toFixed(0);
      }
    };
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        document.getElementById("staked-msg").innerHTML = `
                <b><span style="font-size: 14px">Total Staked: </span></b> ${parseFloat(
                  xhttp.responseText
                ).toFixed(4)} Ichips`;
      }
    };
...
function getUserBalance(account) {
  try {
    fetch("https://api.iost.io/getTokenBalance/" + account + "/hodl/true")
      .then((res) => res.json())
      .then((json) => {
        document.getElementById("user-iChip-balance").innerHTML = `
                                <b><span style="font-size: 14px">Your Wallet: </span></b> <span class="timer-default number-empathy">${parseFloat(
                                  json.balance
                                ).toFixed(4)} </span>HODL`;
      })
      .catch((err) => {
        document.getElementById("user-iChip-balance").innerHTML = `
                                <b><span style="font-size: 14px">Your Wallet: </span></b> <span class="timer-default number-empathy">${(0).toFixed(
                                  4
                                )} </span>HODL`;
      });
    fetch("https://api.iost.io/getTokenBalance/" + account + "/iost/true")
      .then((res) => res.json())
      .then((json) => {
          ...
          window.IWalletJS.enable()
            .then((account) => {
              ...
              document.getElementById("user-staked-balance").innerHTML = `
                                <b><span style="font-size: 14px">You Staked: </span></b> <span class="timer-default number-empathy">${parseFloat(
                                  user.balance
                                ).toFixed(4)} </span>HODL`;
            })
            .catch((err) => {
              document.getElementById("user-staked-balance").innerHTML = `
                                <b><span style="font-size: 14px">You Staked: </span></b> <span class="timer-default number-empathy">${(0).toFixed(
                                  4
                                )} </span>HODL`;
              return;
            });
          ...
          if (i <= 9) {
            tablebody1 += `
                        <tr>
                            ...
                            <td data-label="ADDRESS"><b>${
                              richlist[i].account
                            }***</b></td>
                            <td data-label="AMOUNT">${(
                              richlist[i].balance * 1
                            ).toFixed(4)}</td>
                            <td data-label="HOLDING">${(
                              richlist[i].percent * 100
                            ).toFixed(2)} %</td>
                        </tr>
                        `;
          } else if (i > 9 && i < 20) {
...
$(document).on("click", "#buyBtn", function () {
  if (isTimerValid) {
    if (!window.IWalletJS) {
      $("#statusBuyMsg").html(
        '<div class="alert alert-warning">You need to install <a style="color: #fcc56e;" href="https://chrome.google.com/webstore/detail/iwallet/kncchdigobghenbbaddojjnnaogfppfj">iWallet Chrome Extension</a>.</div>'
      );
      return;
    }
      ...
      .then(function (val) {
        ...
        if (tokenAmount) {
          ...
          const tx = iost.callABI(
            "ContractDYPoVRRYvRBbJGoBGfSY1TBmkT7AwDFAUWTbi3sFAa3E",
            "buyToken",
          ...
          iost
            ...
            .on("success", function (result) {
              ...
              $(".page-loader").hide();
              $("#statusBuyMsg").html(
                '<div class="alert alert-success">Successfully purchased. Please check your wallet</div>'
              );
            })
          ...
          $("#statusBuyMsg").html(
            '<div class="alert alert-warning">Please input purchase amount.</div>'
          );
        }
      })
      .catch((error) => {
        if (error.type == "locked")
          $("#statusBuyMsg").html(
            '<div class="alert alert-warning">Unlock your iWallet Extension.</div>'
          );
      });
  }
});
        ...
        if (tokenAmount) {
          ...
          iost
            ...
            .on("success", function (result) {
              ...
              $(".page-loader").hide();
              $("#statusStakeMsg").html(
                '<div class="alert alert-success">Successfully staked. Please check your wallet</div>'
              );
              getRichList();
              getTotalStaked();
            })
          ...
          $("#statusStakeMsg").html(
            '<div class="alert alert-warning">Please input stake amount.</div>'
          );
        }
      })
      .catch((error) => {
        ...
        if (tokenAmount) {
          ...
          iost
            ...
            .on("success", function (result) {
              ...
              $(".page-loader").hide();
              $("#statusStakeMsg").html(
                '<div class="alert alert-success">Successfully unstaked. Please check your wallet</div>'
              );
              getRichList();
              getTotalStaked();
            })
          ...
          $("#statusStakeMsg").html(
            '<div class="alert alert-warning">Please input unstake amount.</div>'
          );
        }
      })
      .catch((error) => {
